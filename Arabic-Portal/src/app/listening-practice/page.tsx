...
import { listeningPractice, ListeningPracticeOutput } from "@/ai/flows/listening-practice";
import { useToast } from "@/hooks/use-toast";
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group";

const ListeningPracticePage = () => {
  const [level, setLevel] = useState("B1");
  const [practiceData, setPracticeData] = useState<ListeningPracticeOutput | null>(null);
  const [selectedAnswerIndex, setSelectedAnswerIndex] = useState<number | null>(null);
  const { toast } = useToast();

  const handleStartPractice = async () => {
    const result = await listeningPractice({ level });
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          <div className="grid gap-2">
            ...
            <RadioGroup defaultValue={level} className="flex flex-row space-x-2" onValueChange={(value) => setLevel(value)}>
      ...
      {practiceData && (
        <Card>
          ...
          <CardContent className="space-y-4">
            <audio controls src={practiceData.audioUrl}>
